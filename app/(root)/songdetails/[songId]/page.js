...
const SongDetails = async ({ params }) => {
  const response = await fetch(`https://saavn.me/songs?id=${params.songId}`);
  const result = await response.json();
  const data = result.data[0];

  return (
    <div className="w-full h-[80vh] flex flex-col md:flex-row  gap-x-4 text-white rounded-lg shadow-md mb-[15rem]">
      <Image
      ...
      <div className="flex justify-start items-start flex-col gap-y-2 w-full">
        ...
        <p className="copyright text-sm font-sans text-gray-400 text-ellipsis line-clamp-1">{data.copyright}</p>
      </div>
    </div>
  );
};
